...
- `references/repo-routing-metadata.json` when updating router placement or comparing scenario signals.

All commands below are cwd-independent when they use an absolute skill root. Set it once if convenient:

```bash
export ICEDIT_SKILL=/root/github-repos/production_batches/batch_10/ICEdit/skills/disco/ic-edit
```
