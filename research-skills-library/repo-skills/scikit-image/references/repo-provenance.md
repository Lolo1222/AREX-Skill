...
- Local source path during analysis: `/root/github-repos/production_batches/batch_5/scikit-image`
- Branch: `main`
...
The files under `skills/disco/scikit-image/` are intended to operate without access to the original repository. Runtime guidance, route boundaries, troubleshooting, and the safe I/O smoke helper are bundled in the skill tree. The local path above is provenance only; agents must not require it to execute ordinary user workflows.
