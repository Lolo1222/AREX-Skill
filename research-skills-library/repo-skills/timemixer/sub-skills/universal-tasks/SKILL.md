...
description: "Use and debug TimeMixer imputation, anomaly detection, and classification workflows."
...
- Building or checking `run.py` commands for imputation, anomaly detection, or classification, using the explicit helper path `/root/github-repos/production_batches/batch_10/TimeMixer/skills/disco/timemixer/sub-skills/universal-tasks/scripts/build_universal_task_command.py`.
...
- Debugging dataset-name, mask, threshold, or classification shape issues that are specific to these tasks.
  The helper prints commands only; run generated `run.py` commands from `/root/github-repos/production_batches/batch_10/TimeMixer`, not from the skill directory.
## Safe defaults
