...
description: "Use and debug the TimeMixer Model API, PDM/FMM architecture, decomposition, downsampling, channel independence, normalization, and task-specific forward shapes."
...
4. Run the bundled smoke helper when you need a fast CPU-only shape check. Invoke the helper by its explicit skill path, while pointing `--repo-root` at the source checkout:
   ...
   ```bash
   cd /root/github-repos/production_batches/batch_10/TimeMixer
   python /root/github-repos/production_batches/batch_10/TimeMixer/skills/disco/timemixer/sub-skills/model-architecture/scripts/smoke_timemixer_forward.py --repo-root /root/github-repos/production_batches/batch_10/TimeMixer --task long_term_forecast
   ```
