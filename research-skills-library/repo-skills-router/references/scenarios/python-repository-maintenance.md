...
Choose this scenario when the user is contributing to or modifying a repo checkout; otherwise choose the user-facing workflow scenario for the package. Choose openhands when the user is contributing to or troubleshooting the OpenHands repository itself, especially tasks involving app-server APIs, frontend settings or chat UI, enterprise SaaS code, OpenHands skills/microagents, validation commands, lockfiles, or issue automation.
